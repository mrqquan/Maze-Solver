from graphics import Window,Line,Point,Cell
import time
import random

class Maze():
    def __init__(self,x1,y1,num_rows,num_cols,cell_size_x,cell_size_y,win=None,seed=None):
        self._x1 = x1
        self._y1 = y1
        self._num_rows = num_rows
        self._num_cols = num_cols
        self._cell_size_x = cell_size_x
        self._cell_size_y = cell_size_y
        self._win = win
        self._cells = []
        random.seed(seed)

        print("Creating Cells...")
        self._create_cells()
        
        print("Creating Entrance and Exit...")
        self._break_entrance_and_exit()
        
        print("Breaking the Walls...")
        self._break_walls_r(0,0)

        print("Finalising the Maze...")
        self._reset_cells_visited()
    
    def _create_cells(self):
        for col in range(self._num_cols):
            lst = []
            for row in range(self._num_rows):
                lst.append(Cell(self._win))
            self._cells.append(lst)
        for col in range(len(self._cells)):
            for row in range(len(self._cells[col])):
                self._draw_cell(col,row)
                self._animate()
                

    def _draw_cell(self,i,j):
        if self._win is None:
            return
        cell_x = self._x1 + (i * self._cell_size_x)
        cell_y = self._y1 + (j * self._cell_size_y)
        self._cells[i][j].draw(cell_x, cell_y, cell_x+self._cell_size_x, cell_y + self._cell_size_y )
    
    def _break_entrance_and_exit(self):
        self._cells[0][0].has_top_wall = False
        self._draw_cell(0,0)
        self._cells[-1][-1].has_bottom_wall = False
        self._draw_cell(self._num_cols-1,self._num_rows-1)
    
    def _animate(self):
        if self._win is None:
            return
        self._win.redraw()
        time.sleep(0.02)
    
    
    def _break_walls_r(self,i,j):
        self._cells[i][j].visited = True
        while True:
            lst = []

            if i+1 < self._num_cols and not self._cells[i+1][j].visited:
                lst.append(["right",i+1,j])
            
            if i-1 >= 0 and not self._cells[i-1][j].visited:
                lst.append(["left",i-1,j])
            
            if j+1 < self._num_rows and not self._cells[i][j+1].visited:
                lst.append(["bottom",i,j+1])
            
            if j-1 >= 0 and not self._cells[i][j-1].visited:
                lst.append(["top",i,j-1])
            
            if len(lst) == 0:
                self._draw_cell(i,j)
                self._animate()
                return
            else:
            
                chosen = random.choice(lst)
                if chosen[0] == "right":
                    self._cells[chosen[1]][chosen[2]].has_left_wall = False
                    self._cells[i][j].has_right_wall = False
                elif chosen[0] == "left":
                    self._cells[chosen[1]][chosen[2]].has_right_wall = False
                    self._cells[i][j].has_left_wall = False
                elif chosen[0] == "top":
                    self._cells[chosen[1]][chosen[2]].has_bottom_wall = False
                    self._cells[i][j].has_top_wall = False
                elif chosen[0] == "bottom":
                    self._cells[chosen[1]][chosen[2]].has_top_wall = False
                    self._cells[i][j].has_bottom_wall = False

                self._break_walls_r(chosen[1],chosen[2])
    
    def _reset_cells_visited(self):
        for col in range(self._num_cols):
            for row in range(self._num_rows):
                self._cells[col][row].visited = False  
    
    def solve(self):
        return self._solve_r(0,0)

    
    def _solve_r(self,i=0,j=0):
        self._animate()
        self._cells[i][j].visited = True

        if i == self._num_cols - 1 and j == self._num_rows - 1:
            return True

        # move left    
        if i > 0 and self._cells[i][j].has_left_wall is False and not self._cells[i-1][j].visited:
            self._cells[i][j].draw_move(self._cells[i-1][j])
            if self._solve_r(i-1,j):
                return True
            else:
                self._cells[i][j].draw_move(self._cells[i-1][j],True)

        # move right
        if i < self._num_cols-1 and self._cells[i][j].has_right_wall is False and not self._cells[i+1][j].visited:
            self._cells[i][j].draw_move(self._cells[i+1][j])
            if self._solve_r(i+1,j):
                return True
            else:
                self._cells[i][j].draw_move(self._cells[i+1][j],True)

        # move up
        if j > 0 and self._cells[i][j].has_top_wall is False and not self._cells[i][j-1].visited:
            self._cells[i][j].draw_move(self._cells[i][j-1])
            if self._solve_r(i,j-1):
                return True
            else:
                self._cells[i][j].draw_move(self._cells[i][j-1],True)
        
        # move down    
        if j < self._num_rows-1 and self._cells[i][j].has_bottom_wall is False and not self._cells[i][j+1].visited:
            self._cells[i][j].draw_move(self._cells[i][j+1])
            if self._solve_r(i,j+1):
                return True
            else:
                self._cells[i][j].draw_move(self._cells[i][j+1],True)
            
        
        
        return False
        
    """
    def _solve_r(self, i, j):
        self._animate()

        # vist the current cell
        self._cells[i][j].visited = True

        # if we are at the end cell, we are done!
        if i == self._num_cols - 1 and j == self._num_rows - 1:
            return True

        # move left if there is no wall and it hasn't been visited
        if (
            i > 0
            and not self._cells[i][j].has_left_wall
            and not self._cells[i - 1][j].visited
        ):
            self._cells[i][j].draw_move(self._cells[i - 1][j])
            if self._solve_r(i - 1, j):
                return True
            else:
                self._cells[i][j].draw_move(self._cells[i - 1][j], True)

        # move right if there is no wall and it hasn't been visited
        if (
            i < self._num_cols - 1
            and not self._cells[i][j].has_right_wall
            and not self._cells[i + 1][j].visited
        ):
            self._cells[i][j].draw_move(self._cells[i + 1][j])
            if self._solve_r(i + 1, j):
                return True
            else:
                self._cells[i][j].draw_move(self._cells[i + 1][j], True)

        # move up if there is no wall and it hasn't been visited
        if (
            j > 0
            and not self._cells[i][j].has_top_wall
            and not self._cells[i][j - 1].visited
        ):
            self._cells[i][j].draw_move(self._cells[i][j - 1])
            if self._solve_r(i, j - 1):
                return True
            else:
                self._cells[i][j].draw_move(self._cells[i][j - 1], True)

        # move down if there is no wall and it hasn't been visited
        if (
            j < self._num_rows - 1
            and not self._cells[i][j].has_bottom_wall
            and not self._cells[i][j + 1].visited
        ):
            self._cells[i][j].draw_move(self._cells[i][j + 1])
            if self._solve_r(i, j + 1):
                return True
            else:
                self._cells[i][j].draw_move(self._cells[i][j + 1], True)

        # we went the wrong way let the previous cell know by returning False
        return False
        """




    
    
